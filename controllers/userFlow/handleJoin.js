async function handleJoin(senderId, db) {
  return db.get(
    ...
    async (err, row) => {
      try {
        if (err) {
          console.error(`Помилка БД в handleJoin для користувача ${senderId}:`, err);
        ...
        if (latinName) {
          ...
          return sendQuickReplies(
            senderId,
            'Quel type ou format souhaitez-vous imprimer ? (ex : A4, carte...)',
            returnButton
          );
        } else {
          ...
          return sendQuickReplies(
            senderId,
            '🖊️ Veuillez saisir votre nom complet (ou un nom pour l’identification) :',
            returnButton
          );
        }
      } catch (error) {
        console.error(`Непередбачена помилка в handleJoin для користувача ${senderId}:`, error);
        await sendMessage(senderId, '❌ Une erreur inattendue est survenue. Veuillez réessayer plus tard.');
        return sendQuickReplies(senderId, '🔽 Que souhaitez-vous faire ?', returnButton);
      }
    }
  );
